...
const ROUNDS = 8;               // 문장 8개
const MIN_ACC = 0.6;            // 이 정확도 미만은 미완주 처리(0점)
// 순위별 기본 배점 (반응속도와 동일)
const RANK_PTS = [100, 80, 65, 55, 45, 40, 35, 30, 25, 20, 15, 10];
// ⚖️ 밸런스: 배점이 라운드 수에 그대로 비례하므로, 라운드가 늘어도 게임 총점이
//    다른 게임과 맞도록 3라운드 기준으로 배율을 자동 조정한다.
const BASELINE_ROUNDS = 3;
const RANK_SCALE = BASELINE_ROUNDS / ROUNDS;
function rankPts(order) { return Math.max(5, Math.round((RANK_PTS[order] || 5) * RANK_SCALE)); }

function shuffle(a) { for (let i = a.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1));[a[i], a[j]] = [a[j], a[i]]; } return a; }

// 비교용 정규화: 앞뒤 공백 제거 + 연속 공백 1칸 (띄어쓰기 실수는 관대하게, 글자는 엄격하게)
// ⚠️ 길이 상한: 편집거리 계산이 O(문장길이 × 입력길이)라, 입력이 비정상적으로 길면
//    서버가 그 시간 동안 멈춘다. 문장이 40자 내외이므로 넉넉히 300자로 자른다.
const MAX_INPUT = 300;
function norm(s) { return String(s || '').slice(0, MAX_INPUT).trim().replace(/\s+/g, ' '); }
