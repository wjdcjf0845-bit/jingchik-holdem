...
const round10 = v => Math.max(50, Math.round(v / 10) * 10);

// 인원 수에 맞춰 물건 개수를 정하고, 총점이 targetTotal이 되도록 가치를 재조정한다.
function buildItems(nPlayers) {
    const pool = shuffle(DATA.items.slice());
    const count = Math.max(DATA.minItems, Math.min(pool.length, Math.round(nPlayers * DATA.itemsPerPlayer)));
    const picked = pool.slice(0, count);
    const rawSum = picked.reduce((s, it) => s + it.value, 0) || 1;
    const scale = DATA.targetTotal / rawSum;
    return picked.map(it => {
        const base = round10(it.value * scale);
        if (it.mystery) {
            // 미스터리: 기준가의 60~140% 사이로 랜덤 공개 (입찰 중엔 숨김)
            return { ...it, value: round10(base * (0.6 + Math.random() * 0.8)) };
        }
...
module.exports = {
    ...
    rules: {
        ...
        steps: [
            ...
            '여러 물건과 가치가 <b>처음부터 모두 공개</b>됩니다 (인원에 맞춰 개수 자동 조정)',
    ...
    create(room, ctx) {
        const players = ctx.connectedPlayers();
        const gs = { items: buildItems(players.length), rIndex: 0, phase: 'intro', budgets: {}, bids: {},
                     result: null, deadline: 0, _pushClock: false, budget: DATA.budget };
        gs.total = gs.items.length;
        for (const p of players) gs.budgets[p.id] = DATA.budget;
        return gs;
    },
    ...
    action(room, gs, player, msg, ctx) {
        ...
        const connected = ctx.awaitedPlayers();
