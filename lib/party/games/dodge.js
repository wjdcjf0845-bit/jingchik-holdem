...
// ── 난이도: 5초마다 한 단계씩 상승 (계단식) ──
const LEVEL_SEC = 5;
const levelOf = t => Math.floor(Math.max(0, t) / LEVEL_SEC);   // 0,1,2,...
const wavesAt = lv => 1 + Math.floor(lv / 2);                  // 동시 발사 수 1→5
const gapAt   = lv => Math.max(0.30, 1.00 - lv * 0.09);        // 발사 간격 1.00→0.30초
const speedAt = lv => 1 + lv * 0.085;                          // 총알 속도 배율 1.0→1.7
...
function spawnWarning(gs, lv, rnd) {
    ...
    const sp = BULLET_SPEED * (0.85 + rnd() * 0.4) * speedAt(lv);
...
module.exports = {
    ...
    rules: {
        ...
        tips: [
            ...
            '<b>' + LEVEL_SEC + '초마다 난이도가 한 단계씩 올라갑니다</b> (총알이 많아지고 빨라져요)',
    ...
    onTick(room, gs, ctx, toHost) {
        ...
        // 2) 경고선 생성 — 난이도는 5초마다 한 단계씩 상승
        const lv = levelOf(t);
        if (lv !== gs.level) { gs.level = lv; gs.levelUpAt = now; }   // TV에 레벨업 연출용
        const rnd = Math.random;
        while (t >= gs.nextSpawn) {
            const waves = wavesAt(levelOf(gs.nextSpawn));
            for (let i = 0; i < waves; i++) spawnWarning(gs, levelOf(gs.nextSpawn), rnd);
            gs.nextSpawn += gapAt(levelOf(gs.nextSpawn));
        }
        ...
        toHost('host:arena', {
            ...
            lv: lv + 1,                                   // 화면 표시용 레벨 (1부터)
            up: now - gs.levelUpAt < 1200 ? 1 : 0,        // 방금 올라갔으면 연출
            nextIn: r1(Math.max(0, (lv + 1) * LEVEL_SEC - t)),
            ships: Object.values(gs.ships).map(s => ({
                x: r1(s.x), y: r1(s.y), n: s.nick, c: s.color, d: s.dead ? 1 : 0 })),
            // a = 진행 각도(도) — TV에서 꼬리불꽃 방향을 그리는 데 쓴다
            bullets: gs.bullets.map(b => ({ x: r1(b.x), y: r1(b.y),
                a: Math.round(Math.atan2(b.vy, b.vx) * 180 / Math.PI) })),
