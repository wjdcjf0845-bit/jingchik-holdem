...
const DRAW_MS = 60000;             // 라운드 제한시간 (12명이면 최대 15분 → 12분)
...
// 접속이 끊긴 사람 차례는 건너뛴다 (끊긴 사람이 드로어면 아무도 못 그려 라운드가 죽음)
function isConnectedDrawer(room, gs, i) {
    const p = room.players.get(gs.order[i]);
    return !!(p && p.connected);
}
function beginRoundFrom(room, gs) {
    while (gs.rIndex < gs.total && !isConnectedDrawer(room, gs, gs.rIndex)) gs.rIndex++;
    if (gs.rIndex >= gs.total) return false;
    startRound(gs);
    return true;
}
...
module.exports = {
    ...
    rules: {
        ...
        tips: [
            ...
            '한 라운드 ' + Math.round(DRAW_MS / 1000) + '초',
        ],
    },
    ...
    create(room, ctx) {
        ...
        if (!beginRoundFrom(room, gs)) startRound(gs); // 전원 즉시 이탈 같은 극단 상황 대비
        return gs;
    },
    ...
    action(room, gs, player, msg, ctx) {
        ...
        if (acceptedOf(gs.entry).includes(g)) {
            ...
            const guessers = ctx.awaitedPlayers().filter(p => p.id !== gs.drawerId);
    ...
    // 그리던 사람이 나가면 아무도 그릴 수 없으므로 그 라운드를 바로 정리한다
    onPlayerLeave(room, gs, player, ctx) {
        if (gs.phase === 'draw' && player.id === gs.drawerId) toReveal(gs);
    },

    advance(room, gs, ctx) {
        if (gs.phase === 'draw') { toReveal(gs); return; }
        gs.rIndex++;
        if (!beginRoundFrom(room, gs)) { ctx.finish(); return; }  // 남은 라운드가 없거나 전부 이탈이면 종료
    },
