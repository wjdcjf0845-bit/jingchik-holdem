...
// ⚖️ 밸런스: 곡 수가 그대로 배수가 되므로(곡당 배점 × 곡 수),
//    곡을 추가·삭제해도 다른 게임과 총점이 맞도록 자동으로 나눠 쓴다.
const TARGET_TOTAL = 1560;        // 이 게임 전체가 뿌릴 목표 총점
const RANK_RATIO = [0.5, 0.31, 0.19];  // 1등 / 2등 / 3등 배분 비율
const WINNERS = RANK_RATIO.length;     // 3등까지
const FEED_MAX = 12;              // TV에 띄우는 최근 입력 개수

// 곡 수에 맞춘 순위별 배점 (최소 5점 보장)
function rankPtsFor(songCount) {
    const per = TARGET_TOTAL / Math.max(1, songCount);
    return RANK_RATIO.map(r => Math.max(5, Math.round(per * r / 5) * 5));
}
...
module.exports = {
    ...
    rules: {
        ...
        scoring: [
            '선착순 <b>1등 · 2등 · 3등</b>까지만 점수 (빠를수록 높은 점수)',
            '4등부터는 맞혀도 0점',
            '3명이 맞히면 그 라운드는 즉시 종료',
        ],
        tips: [
            '띄어쓰기·대소문자는 틀려도 정답 처리',
            '영어 제목은 한글로 소리 나는 대로 써도 인정돼요',
            '라운드당 제한시간 2분',
        ],
    },
    ...
    create(room, ctx) {
        ...
        return { songs, qIndex: 0, phase: 'ready', answers: {}, wrongs: {}, feed: [],
                 ...
                 rankPts: rankPtsFor(songs.length) };
    },
    ...
    action(room, gs, player, msg, ctx) {
        ...
        if (acceptedOf(song).includes(text)) {
            ...
            const pts = gs.rankPts[order] || 0;       // 3등까지만 점수
    ...
    hostView(room, gs) {
        ...
        const base = { qNum: gs.qIndex + 1, total: gs.songs.length, connected,
                       ...
                       winners: WINNERS, rankPts: gs.rankPts };
