...
function preflopRangeTier(code, position, facingRaise, ctx) {
    ...
    if (facingRaise) {
        ...
        const callTh = defenseThreshold(numActive, threeBetPlus);
        const raiseTh = threeBetPlus ? 90 : 85; // 추가 3벳/4벳은 프리미엄 위주
        if (inRange) {
            // 차트 내 핸드라도 레이즈 직면 시엔 "디펜스 콜" — 인원·리레이즈 깊이로 임계 조정
            if (score >= raiseTh) return { tier: 'raise', label: '3벳/밸류', score };
            if (score >= callTh) return { tier: 'call', label: '콜', score };  // 콜 디펜스
            return { tier: 'fold', label: '폴드', score };  // 디펜스엔 약함
        }
        // 차트 밖 — 디펜스 임계보다 살짝 높아야 콜(차트 밖이라 약간 보수적)
        if (score >= callTh + 4) return { tier: 'call', label: '콜', score };
